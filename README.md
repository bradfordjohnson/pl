...
The `media-ingress.py` script is designed for efficiently extracting, processing, and uploading media file metadata and associated sidecar files. This script leverages the `ExifTool` to extract metadata from various media file types and organizes the extracted data into a structured format for further processing.
...
1. **Metadata Extraction:**
   - Utilizes `ExifTool` to retrieve key tags, including:
     - ExifTool version
...
2. **Checksum Generation:**
   - Computes a SHA-256 checksum for each media file, ensuring data integrity during transfers.

3. **UUID Generation:**
   - Generates a unique identifier (UUID) for each media file, facilitating easy reference and tracking within the system.
...
Each sidecar file is represented by the following attributes:
...
The `MediaFile` class is responsible for handling individual media files. It extracts metadata, generates checksums and UUIDs, and moves files to the target directory. Key methods include:
...
The `SidecarFile` class manages metadata files associated with media files. It extracts data from JSON, CSV, and XMP formats, allowing the user to maintain a cohesive structure across media and its metadata. Key methods include:
...
The `MediaDirectory` class is responsible for scanning a specified directory for media and sidecar files. It organizes the files into appropriate classes and manages the import process. Key methods include:
...
To use the script, specify the target directory containing media and sidecar files, and execute the script. It will automatically scan the directory, process the files, and upload the metadata for each file.

### Note

The current implementation includes print statements for debugging purposes. The integration with FastAPI and PostgreSQL has been initiated but is not yet fully functional.
